// Tapovana — Trekker Micro‑Stay MVP
// Single‑file Flutter app skeleton implementing core flows:
// Splash → Home/Search → Results → Listing → Slot Hold → Checkout → Success → Trips → Profile
// Note: Uses only Flutter SDK (no external packages). Replace mock services with real APIs.
...
void main() => runApp(const TapovanaApp());
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _loadProperties() async {
    try {
      final resp = await http.get(Uri.parse('${ApiConfig.baseUrl}/properties'));
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body) as List<dynamic>;
        remoteProperties = data.map((e) => Property.fromJson(e as Map<String, dynamic>)).toList();
      }
    } catch (e) {
      if (mounted) {
      ...
      body: Container(
        ...
        child: Center(
          child: Column(
            ...
            children: const [
              Icon(Icons.terrain, size: 80, color: AppColors.sage),
              SizedBox(height: 16),
              Text('Tapovana', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold)),
              SizedBox(height: 8),
              Text('A Sacred Pause Between Journeys'),
            ],
          ),
        ),
      ),
